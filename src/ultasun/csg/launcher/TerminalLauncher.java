...
import ultasun.csg.printing.CsgToJtables;
...
public class TerminalLauncher implements Runnable {
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            if (args[0].toLowerCase().startsWith("-headless")) {
                t1 = new TerminalLauncher(true, new File(args[1]));
            } else {
                t1 = new TerminalLauncher(false, new File(args[0]));
            }
        } else {
            System.out.println("Note: provide a CSG file as the single argument to this program and it will load, search, and print in a headless fashion.");
            t1 = new TerminalLauncher(false);
        }
        t1.run();
    }
    ...
    private void headless() {
        try {
            ...
            CsgToJtables.printJobPerColor(csgo, true);
    ...
    private void wizard(Scanner s) {
        ...
        if (askQuestion("Would you like to print out the result?", false, s)) {
            try {
                CsgToJtables.printJobPerColor(csgo, false);

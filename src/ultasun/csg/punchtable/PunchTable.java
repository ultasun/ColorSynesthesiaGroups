...
public class PunchTable {
    ...
    public PunchTable() {
        punchedElements = new ArrayList<>(CORE_ARRAY_LIST_DEFAULT_SIZE);
        punchTableColumnNames = new ArrayList<>();
        anyPunchedIn = false;
    }
    ...
    private int parseTableForColumnNames(Document doc, String primaryKey) {
        ...
        for (Element a : doc.select("table")) {
            ...
            if (candidate.contains(primaryKey)) {
                punchTableColumnNames = candidate;
                return result;
            } else {
                result++;
            }
    ...
    private void parseTablesToArrayList(Document doc, String primaryKey) {
        ...
        for (Element row : dataTable.select("tr")) {
            ...
            PunchTableRow newRow = new PunchTableRow(row.select("td"), punchTableColumnNames, punchesStartAt);
            punchedElements.add(newRow);
    ...
    public ArrayList<PunchTableRow> getRawPunchedInRows() {
        ArrayList<PunchTableRow> result
                = new ArrayList<>(CORE_ARRAY_LIST_DEFAULT_SIZE);

        for (PunchTableRow pttr : punchedElements) {
            if (pttr.isPunchedIn()) {
                result.add(pttr);
            }
        }
    ...
    public ArrayList<String> getRawStringRows() {
        ArrayList<String> result = new ArrayList<>(CORE_ARRAY_LIST_DEFAULT_SIZE);
    ...
    public ArrayList<PunchTableRow> getRowsForMultipleAttributeMembers(Map<String, String> attributes) {
        ArrayList<PunchTableRow> result = new ArrayList<>(CORE_ARRAY_LIST_DEFAULT_SIZE);
        boolean willAdd;
        for (PunchTableRow pttr : punchedElements) {
            willAdd = true;
    ...
    /**
     * The List of all rows found in the original table.
     */
    private final List<PunchTableRow> punchedElements;

    /**
     * This List is not to be simply copied from the hints, but instead the
     * original header names as found on the web-page.
     */
    private List<String> punchTableColumnNames;

    
    private boolean anyPunchedIn;
    /**
     * Default size of ArrayList to keep up with performance expectations.
     */
    public static int CORE_ARRAY_LIST_DEFAULT_SIZE = 1000;

...
/**
 * For each color, it's going to addResultToPrint out a list of PunchTableRows
 * for each foreignKeyValues option from SEARCH_CRITERIA.
 *
 * So for example, if there is only one key in SEARCH_CRITERIA, for each color
 * group, it will addResultToPrint a list of elements that share the same value
 * for the SEARCH_CRITERIA.
 *
 * @author ultasun
 */
public class PunchTableColorGrouper {
    ...
    /**
     * These attributes make sense to define once, if they needed to be
     * redefined, then you really just need a new ColorGrouper object.
     */
    ...
    public PunchTableColorGrouper(String primaryKey, String fileOrUrl, String orgColorRules, boolean onlyPunchedIn, boolean pkIsName, boolean lastNameNotFirst, boolean includePrimaryKeySearchOption) throws Exception {
        ...
        // Were we given an URL or a file?
        ...
        // Just to be clear, pt is now a table of everybody who is 
        // punched in (because this is a live stream)
        // We still need to color sort by primaryKey
        try {
        ...
        // Now, the PunchTable pt has each of it's PunchTableRows's updated
        // to include the attribute color and the corresponding color group
        // based on the selected primaryKey
    ...
    /**
     * Provide all search queries in a list. The method will parse each
     * attribute set, and add matching PunchTableRow's to the corresponding
     * color List. Duplicates should not exist, however a duplicate will not be
     * added to a given color List.
     *
     * @param allAttributes
     */
